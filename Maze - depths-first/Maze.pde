class Maze {
  ...
  void generateMaze() {
    ...
    Cell initCell = maze[floor(random(0, cols))][floor(random(0, rows))]; // Choose inital Cell
    initCell.visited = true; // Mark as visited
    stack.add(initCell); // Push to stack
    int iteration = 0;
    Cell currentCell;

    // Starts the algorithm until there is no cells left or it times out
    while (stack.size() != 0 && iteration < rows * cols * 2) {
      currentCell = stack.remove(stack.size() - 1); // Pop last cell from stack and make it current cell
      Cell choosenCell = getUnvisitedNeighbour(currentCell);  // Returns a random unvisited neighbour, return current cell if no unvisited neighbours
      //-------------------------------------------------------- Also, the walls get removed inside this function
      if (choosenCell != currentCell) { // Only activates if there is any unvisited neighbours
        stack.add(currentCell); // current cell pushed to stack
        choosenCell.visited = true; // choosen cell set to visited
        stack.add(choosenCell); // choosen cell pushed to stack
      }
      // print(iteration);
      iteration++;
    }
